...
public class StscJavaizer
{
    ...
    static String pickFullJavaClassName(Set usedNames, QName qName, String configname, boolean isDocument, boolean isAttrType)
    {
        ...
        else
        {
            ...
            {
                // add special suffix
                // MOD AR: Add Document suffix only if type with same name exists
                String shortName = base.substring(base.lastIndexOf('.') + 1);
                if (isDocument && state.findGlobalType(new QName(uri, shortName + "Type"), null, null) != null)
                    base = base + "Element";
                else if (isAttrType)
                    base = base + "Attribute";
        ...
        // MOD AR: Remove Type from java name
        if (uniqName.endsWith("Type"))
            uniqName = uniqName.substring(0, uniqName.length()-4);
    ...
    static String pickJavaPropertyName(Set usedNames, String localName, String javaName)
    {
        ...
        // MOD AR: Hack to rename GML "postList" property
        // since it conflicts with list of "pos" properties in all geometry objects
        if (uniqName.equals("PosList"))
            uniqName = "PositionList";
